function results = SRDCF_settings(seq, res_path, bSaveImage, parameters)
...
params.train_gap = 0;                   % The number of intermediate frames with no training (0 corresponds to training every frame)
params.skip_after_frame = 0;           % After which frame number the sparse update scheme should start (1 is directly)
...
params.use_sample_merge = false;                 % Use the generative sample space model to merge samples
...
params.interpolation_method = 'ideal';    % The kind of interpolation kernel
